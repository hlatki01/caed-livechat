...
export class MessageList extends MemoizedComponent {
  ...
  welcomeMessage = "Bem-vindo(a) ao suporte CAEd. Digite algo no campo abaixo para iniciar o atendimento. Obrigado(a)."
  ...
  handleScroll = () => {
    ...
    if (this.base.scrollHeight <= this.base.clientHeight) {
      ...
      this.base.scrollTop + this.base.clientHeight
    ) {
      scrollPosition = MessageList.SCROLL_AT_BOTTOM;
    } else {
  ...
  componentWillUpdate() {
    ...
    if (localStorage.getItem("lastChatToken")) {

      const lastChatToken = localStorage.getItem("lastChatToken")

      if (storage.token > lastChatToken) {
        this.welcomeMessage = "Sua sessão expirou ou foi encerrada. Digite algo no campo abaixo para iniciar o atendimento. Obrigado(a)."
      }
    }
  ...
  renderItems = ({
    ...
    for (let i = 0; i < messages.length; ++i) {
      ...
      if (
        ...
        items.push(
          <JoinCallButton
            callStatus={ongoingCall.callStatus}
            url={url}
      ...
      const showDateSeparator =
        !previousMessage ||
        !isSameDay(parseISO(message.ts), parseISO(previousMessage.ts));
      ...
      items.push(
        <Message
          ...
          compact={
            ...
            message.u &&
            nextMessage.u &&
            message.u._id === nextMessage.u._id &&
            !nextMessage.t
          }
      ...
      const showUnreadSeparator =
        lastReadMessageId && nextMessage && lastReadMessageId === message._id;
  ...
  render = ({ className, style = {} }) => (
    ...
    >
      <ol className={createClassName(styles, "message-list__content")}>
        {this.renderItems(this.props).length > 0 &&
          this.renderItems(this.props)}
        {this.renderItems(this.props).length <= 0 && (
          <div
            className={createClassName(
              styles,
              "message-list__content_no_messages"
            )}
          >
            <li id="pDCiWRFQejoNMWveY" class="message-container__KcRyZ">
              <div class="message-avatars__bpc8g">
                <div
                  aria-label="User picture"
                  class="avatar__zfL8P avatar--nobg__nSXgN message-avatars__avatar__KvWCV"
                >
                  <img
                    src="assets/tecnico-1.png"
                    alt="atendendecaed"
              ...
              <div class="message-content__0O12f">
                <div class="message-bubble__puzVZ">
                  <div class="">
              ...
              <div class="message-time-wrapper__vpb7U">
                <time
                  datetime="2022-08-31T19:32:08.663Z"
                  class="message-time__QMUdG message-time--normal__kn5gI"
                >
                  {this.handleTime()}
                </time>
              </div>
