...
export const closeChat = async ({ transcriptRequested } = {}) => {
	...
	if (token && token.user && token.user.token) {		
		localStorage.setItem("lastChatToken", `${token.user.token}`);
	}
...
const getRoomInfo = async (token) => {
	return await axios.get(`${localStorage.getItem("SERVER_URL")}/api/v1/livechat/room?token=${token}`)
}
